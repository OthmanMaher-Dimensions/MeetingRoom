...
namespace MeetingRoom.Web.Menus;

public class MeetingRoomMenuContributor : IMenuContributor
{
    ...
    {
        var l = context.GetLocalizer<MeetingRoomResource>();

        //Home
        context.Menu.AddItem(
            new ApplicationMenuItem(
                MeetingRoomMenus.Home,
                l["Menu:Home"],
                "~/",
                icon: "fa fa-home",
                order: 1
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                MeetingRoomMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(MeetingRoomPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                MeetingRoomMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(MeetingRoomPermissions.Dashboard.Tenant)
        );
