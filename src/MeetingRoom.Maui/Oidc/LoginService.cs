...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(MeetingRoomConsts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(MeetingRoomConsts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(MeetingRoomConsts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(MeetingRoomConsts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(MeetingRoomConsts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(MeetingRoomConsts.OidcConsts.RefreshTokenKeyName);
    }
}