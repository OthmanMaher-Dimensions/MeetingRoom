...
namespace MeetingRoom.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("MeetingRoom") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "MeetingRoom", DisplayName = "MeetingRoom API", Resources = { "MeetingRoom" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["MeetingRoom_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["MeetingRoom_Web:RootUrl"]!.EnsureEndsWith('/');

            /* MeetingRoom_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["MeetingRoom_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        //MAUI Client
        var mauiClientId = configurationSection["MeetingRoom_Maui:ClientId"];
        if (!mauiClientId.IsNullOrWhiteSpace())
        {
            var mauiClientRootUrl = configurationSection["MeetingRoom_Maui:RootUrl"]?.Replace("_", "-");
            await CreateApplicationAsync(
                name: mauiClientId!,
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["MeetingRoom_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["MeetingRoom_Swagger:RootUrl"]?.TrimEnd('/');
